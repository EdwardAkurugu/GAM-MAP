...
theme_gam_plots <- function(base_size = 12) {
  theme_classic(base_size = base_size) +
    theme(
      ...
      axis.text.y  = element_text(face = "bold", size = base_size),
      axis.title.x = element_text(face = "bold"),
      axis.text.x  = element_text(face = "bold", angle = 0, vjust = 1, size = base_size),
      panel.background = element_rect(fill = "#f0f0f0")
    )
}

theme_autocorr_plots <- function(base_size = 12) {
...
#FOR NO X-AXIS YEAR
theme_residuals1 <- function(base_size = 10) {
  theme_classic(base_size = base_size) +
    theme(
      axis.title.y = element_text(face = "bold", size = base_size),
      axis.text.y  = element_text(face = "bold", size = base_size),
      axis.title.x = element_text(face = "bold"),
      axis.text.x  = element_blank(),   # remove x-axis text (years)
...
# labs(title = input$gamm_diag_region3,
#      x=" ", y="Malaria cases (x10\u00b3)") + theme_residuals1()

# ggsave(
#   filename = paste0("gamm_diag_pacf_", input$gamm_diag_region4, ".tiff"),
#   plot = pacf_plot,
#   device = "tiff",
#   width = 2250,
#   height = 1200,
...
theme_heatmap2 <- function(base_size = 13) {   #rainfall
  theme_classic(base_size = base_size) +
    theme(
      axis.title.y  = element_text(face = "bold", size = base_size),
      axis.text.y   = element_text(face = "bold", size = base_size),
...
theme_heatmap3 <- function(base_size = 13) {   #temperature
  theme_classic(base_size = base_size) +
    theme(
      axis.title.y  = element_text(face = "bold", size = base_size),
      axis.text.y   = element_text(face = "bold", size = base_size),
